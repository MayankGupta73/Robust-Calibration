...
aug_transforms = tfm.Compose([tfm.ToPILImage(),
                            ...
                            #tfm.GaussianBlur((5, 5), sigma=(0.01, 1.0)),
                            tfm.ToTensor()])
...
def get_train_valid_test_loader(args, tta = False, to_blur=False, blur_kernel_size=(65,65), sigma=16):
    ...
    if to_blur:
        train_dataset = GbDataset(img_dir, df, train_labels, blur_kernel_size=blur_kernel_size, sigma=sigma, img_transforms=img_transforms)
    else:
        train_dataset = GbDataset(img_dir, df, train_labels, to_blur=False, img_transforms=img_transforms)
                
    train_loader = DataLoader(train_dataset, batch_size=args.train_batch_size, shuffle=True, num_workers=1)
    
    if tta:
        val_t = aug_transforms
        val_dataset = GbCropDataset(img_dir, df, val_labels, to_blur=False, img_transforms=val_t)
