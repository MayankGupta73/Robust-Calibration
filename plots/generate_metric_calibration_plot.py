...
plot_name = "metric_plot_calibration-ablation"
plot_path =  plot_name + '_' + eval_mode + '-ci_' + gece_distr + '_sigma-' + str(gece_sigma) + '.png'

checkpoints = ["checkpoint/gbc_usg/gbcnet_lr-0.003_nll.pth", "checkpoint/gbc_usg/gbcnet_lr-0.003_dca.pth", "checkpoint/gbc_usg/gbcnet_lr-0.003_fl.pth", "checkpoint/gbc_usg/gbcnet_lr-0.003_fl-mdca.pth", "checkpoint/gbc_usg/gbcnet_lr-0.003_flsd.pth", "checkpoint/gbc_usg/gbcnet_lr-0.003_fl-gmdca4.pth"]
...
if __name__ == "__main__":
    ...
    for n in range(num_checkpoints):
        ...
        print("Eval ", n, checkpoints[n])
        file_logs.write("Eval " + str(n) + checkpoints[n] + "\n")
    ...
    #logging.info(f"Using model : {args.model}")
        ...
        if args.model == "gbcnet":
            model.net.load_state_dict(torch.load(checkpoint, map_location = 'cpu')['state_dict'])
        ...
        checkpoints_rece.append(gece_mean_output)
        checkpoints_rece_m.append(gece_sd_mean_output)

    data_metrics = pd.DataFrame({
        'Data Size': batch_list_output, 
        'CE': checkpoints_rece_m[0],
        'DCA': checkpoints_rece_m[1],
        'FL': checkpoints_rece_m[2],
        'FL+MDCA': checkpoints_rece_m[3],
        'FLSD': checkpoints_rece_m[4],
        'RCR': checkpoints_rece_m[5],
    })
