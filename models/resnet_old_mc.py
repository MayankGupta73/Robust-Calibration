...
class ResNetMC(nn.Module):

    def __init__(self, depth, num_classes=1000, block_name='BasicBlock', dropout_rate=0.1, **kwargs):
...
def resnet20(**kwargs):
    model = ResNetMC(depth=20, **kwargs)
    return model

def resnet32(**kwargs):
    model = ResNetMC(depth=32, **kwargs)
    return model

def resnet56_mc(**kwargs):
    model = ResNetMC(depth=56, block_name="bottleneck", dropout_rate=0.1, **kwargs)
    return model
