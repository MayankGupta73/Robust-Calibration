...
def gbcnet_model(pretrained=False, basewidth=26, scale=4, att_position=[[],[],[],[]], att_dim = 128, **kwargs):
    model = GbcNetModel(Bottleneck, [3, 4, 6, 3], basewidth, scale, att_position, att_dim, **kwargs)
    return model
...
class GbcNet(nn.Module):
    def __init__(self, num_cls=3, last_layer=False, pretrain=False, att_mode="1", head="2"):
        ...
        #self.net = GbcNet(Bottleneck, [3, 4, 6, 3], 26, 4, attpos, 128, GSoP_mode = 1)
        self.net = gbcnet_model(pretrained=pretrain, att_position=attpos, GSoP_mode=1)
...
def gbcnet(num_classes = 3):
    net = GbcNet(num_cls=num_classes, pretrain=True, att_mode="1") 
    return net
...
def resnet50_gbc(num_classes = 2):
    net = Resnet50(num_cls=num_classes, pretrain=True)
    return net