...
def _resnet_dropout(block: Type[Union[BasicBlock, Bottleneck]], layers: List[int], dropout_rate: float, num_classes: int , **kwargs: Any) -> ResNetMCDropout:
  model = ResNetMCDropout(block, layers, dropout_rate, num_classes = num_classes, **kwargs)
  return model


def resnet50_mc_dropout(num_classes = 3, dropout_rate: float = 0.1, **kwargs: Any) -> ResNetMCDropout:
  r"""ResNet-50 v1.5.
  Model from
  ...
  Returns:
    Model.
  """
  return _resnet_dropout(Bottleneck, [3, 4, 6, 3], dropout_rate, num_classes, **kwargs)
  
class Resnet50MC(nn.Module):
  def __init__(self, num_cls=3, dropout_rate = 0.1, pretrain=True):
      super(Resnet50MC, self).__init__()
...
def resnet50_mc_dropout_2(num_classes = 3, dropout_rate = 0.1):
    net = Resnet50MC(num_cls=num_classes, dropout_rate = dropout_rate, pretrain=True)
    return net